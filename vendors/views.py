...
def pool_csv(request):
    ...
    if 'setasides' in request.GET:
        ...
        for sobj in setaside_objs:
            vendors = vendors.objects.filter(setasides=sobj)
...
def vendors_csv(request, vendors_duns):
    vendor = Vendors.objects.get(duns=vendors_duns)
    ...
    writer.writerow(('CAGE Code', vendor.cage, '', '',  titlecase(v.sam_citystate[0:vendor.sam_citystate.index(',') + 1]) + vendor.sam_citystate[vendor.sam_citystate.index(',') + 1:]))
    ...
    if naics:
        contracts = Contract.objects.filter(vendors=vendor, NAICS=naics).order_by('-date_signed')
    else:
        contracts = Contract.objects.filter(vendors=vendor).order_by('-date_signed')
    for c in contracts:
        if '_' in c.piid:
