            ...
            for f in os.listdir(doc_dir):
                ...
                try:
                    ...
                    for idx, line in enumerate(reader): 
                        ...
                        poolpiid, ppcreated = PoolPIID.objects.get_or_create(Vendors=new_obj, pool=pool_obj, piid=piid)

                        if created:
                            self.logger.debug("Successfully created {}".format(new_obj.name))
                        else:
                            self.logger.debug("Vendors {} already in database".format(new_obj.name))
